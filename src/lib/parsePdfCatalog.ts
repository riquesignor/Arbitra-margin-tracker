  ...
  try {
    for (let pageNum = from; pageNum <= to; pageNum++) {
      ...
      if (grid) {
        ...
        // ⚠️ REVERTIDO (set/2026) — 2ª passada de OCR na faixa do preço
        // (auditoria item 18). Ficou pronta e passou nos testes, mas em
        // catálogo REAL de grade densa (Issam, 4x3 por página) produziu
        // preço errado: a faixa é calculada sobre `yTop/yBottom` do bloco
        // de TEXTO, que numa grade apertada invade o cartão vizinho — o
        // maior número lido na tira podia ser o preço do produto de
        // baixo. E, pior que o preço errado, ela REPROMOVIA cartões que o
        // parser tinha descartado como ambíguos: produto que antes ficava
        // de fora passou a entrar na planilha com nome/preço trocados.
        //
        // Errar preço de custo é pior que não ter preço: contamina margem
        // e recomendação de compra em silêncio. Pra voltar, a passada
        // precisa de (a) faixa derivada do RECORTE do cartão, não do bloco
        // de texto, (b) exigir "R$" no texto lido, e (c) validar contra a
        // mediana de preço da própria página antes de aceitar.
        const remainingPriceless = grid.priceless;
      ...
      const geminiHint = !options?.geminiApiKey
        ...
        : geminiQuotaExhausted
          ...
          : " Tentamos ler via IA como último recurso, mas ela também não conseguiu identificar " +
            "produto nenhum nas páginas processadas.";
